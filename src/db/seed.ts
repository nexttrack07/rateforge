...
const platformRows = [
	...
	},
	{
		...
		logoUrl: "https://www.dzine.ai/favicon.ico",
		pricingUrl: "https://www.dzine.ai/pricing",
		shortDescription:
			"AI image and video platform with access to top models like Kling, Seedance, and Midjourney.",
		pros: [
			"Unlimited images on higher tiers",
			"Wide model selection (Kling, Veo, Sora, Midjourney)",
			"Credit rollover up to 3 months",
			"Both image and video generation",
		],
		cons: [
			"Video credits limited even on top tiers",
			"No API access",
			"Fair usage policy on unlimited plans",
		],
	},
	{
		slug: "artlist",
		...
		logoUrl: "https://artlist.io/favicon.ico",
		pricingUrl: "https://artlist.io/pricing",
		shortDescription:
			"Creative suite with AI video, image, music, and voiceover generation for content creators.",
		pros: [
			"All-in-one creative suite (video, image, music, voice)",
			"Access to Sora, Veo, Kling, Seedance",
			"Credits shared across all AI tools",
			"Studio for end-to-end production",
		],
		cons: [
			"Higher entry price than competitors",
			"Credits don't roll over",
			"Focused on professional creators",
		],
	},
];
...
async function main() {
	...
	const insertedPlans = await database
		...
		.values([
			...
			// Dzine plans (from https://www.dzine.ai/pricing)
			{
				platformId: platformIdBySlug.get("dzine")!,
				name: "Creator",
				priceMonthly: "24.99",
				credits: 6000, // 3,000 image + 3,000 video credits
			},
			{
				platformId: platformIdBySlug.get("dzine")!,
				name: "Master",
				priceMonthly: "59.99",
				credits: 9000, // Unlimited image (fair use) + 9,000 video credits
			},
			{
				platformId: platformIdBySlug.get("dzine")!,
				name: "Master Pro",
				priceMonthly: "149.99",
				credits: 30000, // Unlimited image (fair use) + 30,000 video credits
			},
			// Artlist plans (from https://artlist.io/pricing)
			{
				platformId: platformIdBySlug.get("artlist")!,
				name: "AI Starter",
				priceMonthly: "11.99",
				credits: 16500, // ~1,650 images or ~103 videos
			},
			{
				platformId: platformIdBySlug.get("artlist")!,
				name: "AI Professional",
				priceMonthly: "89.99",
				credits: 180000, // ~18,000 images or ~1,125 videos
			},
		])
		.returning({
			id: platformPlans.id,
	...
	// Dzine pricing (subscription-based credits)
	// Using Creator plan: $24.99/6000 credits = $0.00417/credit
	console.log("[db:seed] inserting Dzine platform pricing");
	const dzineId = platformIdBySlug.get("dzine")!;
	const dzineCreatorPlanId = planIdByKey.get(`${dzineId}:Creator`)!;
	const dzineCreditPrice = 24.99 / 6000; // $0.00417 per credit

	await database.insert(platformModelPricing).values([
		// IMAGE MODELS on Dzine (Creator plan)
		// Nano Banana Pro 4K: 40 credits/image
		{
			platformId: dzineId,
			platformPlanId: dzineCreatorPlanId,
			modelId: modelIdBySlug.get("nano-banana-pro")!,
			creditsPerGen: 40,
			priceUsd: (40 * dzineCreditPrice).toFixed(4),
			resolution: "4K",
		},
		// Nano Banana Pro 2K: 20 credits/image
		{
			platformId: dzineId,
			platformPlanId: dzineCreatorPlanId,
			modelId: modelIdBySlug.get("nano-banana-pro")!,
			creditsPerGen: 20,
			priceUsd: (20 * dzineCreditPrice).toFixed(4),
			resolution: "2K",
		},
		// Seedream 4.5 4K: 10 credits/image
		{
			platformId: dzineId,
			platformPlanId: dzineCreatorPlanId,
			modelId: modelIdBySlug.get("seedream-4-5")!,
			creditsPerGen: 10,
			priceUsd: (10 * dzineCreditPrice).toFixed(4),
			resolution: "4K",
		},
		// Seedream 4.5 2K: 8 credits/image
		{
			platformId: dzineId,
			platformPlanId: dzineCreatorPlanId,
			modelId: modelIdBySlug.get("seedream-4-5")!,
			creditsPerGen: 8,
			priceUsd: (8 * dzineCreditPrice).toFixed(4),
			resolution: "2K",
		},
		// GPT Image 1.5: estimated ~25 credits/image
		{
			platformId: dzineId,
			platformPlanId: dzineCreatorPlanId,
			modelId: modelIdBySlug.get("gpt-image-1-5")!,
			creditsPerGen: 25,
			priceUsd: (25 * dzineCreditPrice).toFixed(4),
			resolution: "1K",
		},
		// VIDEO MODELS on Dzine (Creator plan) - video credits
		// Kling 2.6/3.0 1080p 5s: ~86 credits
		{
			platformId: dzineId,
			platformPlanId: dzineCreatorPlanId,
			modelId: modelIdBySlug.get("kling-3-0")!,
			creditsPerGen: 86 / 5, // per second
			priceUsd: ((86 / 5) * dzineCreditPrice).toFixed(4),
			resolution: "1080p",
		},
		// Seedance Pro 1080p 5s: ~120 credits
		{
			platformId: dzineId,
			platformPlanId: dzineCreatorPlanId,
			modelId: modelIdBySlug.get("seedance-2-0")!,
			creditsPerGen: 120 / 5, // per second
			priceUsd: ((120 / 5) * dzineCreditPrice).toFixed(4),
			resolution: "1080p",
		},
		// Google Veo 3.1 Fast 1080p 8s: ~200 credits
		{
			platformId: dzineId,
			platformPlanId: dzineCreatorPlanId,
			modelId: modelIdBySlug.get("google-veo-3-1-fast")!,
			creditsPerGen: 200 / 8, // per second
			priceUsd: ((200 / 8) * dzineCreditPrice).toFixed(4),
			resolution: "1080p",
		},
		// Wan 2.6 1080p 5s: ~107 credits
		{
			platformId: dzineId,
			platformPlanId: dzineCreatorPlanId,
			modelId: modelIdBySlug.get("wan-2-7-video")!,
			creditsPerGen: 107 / 5, // per second
			priceUsd: ((107 / 5) * dzineCreditPrice).toFixed(4),
			resolution: "1080p",
		},
		// Sora 2: estimated ~400 credits for 5s
		{
			platformId: dzineId,
			platformPlanId: dzineCreatorPlanId,
			modelId: modelIdBySlug.get("sora-2")!,
			creditsPerGen: 400 / 5, // per second
			priceUsd: ((400 / 5) * dzineCreditPrice).toFixed(4),
			resolution: "720p",
		},
	]);

	// Artlist pricing (subscription-based credits)
	// Using AI Professional plan: $89.99/180000 credits = $0.0005/credit
	console.log("[db:seed] inserting Artlist platform pricing");
	const artlistId = platformIdBySlug.get("artlist")!;
	const artlistProPlanId = planIdByKey.get(`${artlistId}:AI Professional`)!;
	const artlistCreditPrice = 89.99 / 180000; // $0.0005 per credit

	await database.insert(platformModelPricing).values([
		// IMAGE MODELS on Artlist (AI Professional plan)
		// ~10 credits per standard image
		{
			platformId: artlistId,
			platformPlanId: artlistProPlanId,
			modelId: modelIdBySlug.get("nano-banana")!,
			creditsPerGen: 10,
			priceUsd: (10 * artlistCreditPrice).toFixed(4),
			resolution: "1K",
		},
		{
			platformId: artlistId,
			...
			modelId: modelIdBySlug.get("gpt-image-1-5")!,
			creditsPerGen: 15,
			priceUsd: (15 * artlistCreditPrice).toFixed(4),
			resolution: "1K",
		},
		// VIDEO MODELS on Artlist (AI Professional plan)
		// ~160 credits per 5s video clip
		// Sora
		{
			platformId: artlistId,
			platformPlanId: artlistProPlanId,
			modelId: modelIdBySlug.get("sora-2")!,
			creditsPerGen: 200 / 5, // per second
			priceUsd: ((200 / 5) * artlistCreditPrice).toFixed(4),
			resolution: "720p",
		},
		// Veo
		{
			...
			platformPlanId: artlistProPlanId,
			modelId: modelIdBySlug.get("google-veo-3-1")!,
			creditsPerGen: 180 / 5, // per second
			priceUsd: ((180 / 5) * artlistCreditPrice).toFixed(4),
			resolution: "1080p",
		},
		// Kling
		{
			...
			platformPlanId: artlistProPlanId,
			modelId: modelIdBySlug.get("kling-3-0")!,
			creditsPerGen: 160 / 5, // per second
			priceUsd: ((160 / 5) * artlistCreditPrice).toFixed(4),
			resolution: "1080p",
		},
		// Seedance
		{
